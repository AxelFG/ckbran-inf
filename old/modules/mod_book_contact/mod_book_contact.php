...
$wrongantispamanswer = $params->get('wrong_antispam', 'Введено неверное проверочное число. ');
...
print '<style type="text/css">
		#book_button {
			border: 1px solid #5577af;
			color: #fff;
			background: #5577af url(/templates/protostar/images/book.png) 0% 50% no-repeat;
			display: block;
			line-height: 36px;
...
//  }
  ...
  if ($myError == '') {
    ...
    if ($mailSender->Send() !== true) {
      ...
      $myReplacement = '<a id="book_button" href="#" onclick="document.getElementById(\'book_form\').style.display = \'block\';document.getElementById(\'book_bg\').style.display = \'block\';">Записаться на приём</a>
	  					<div id="book_message"><a id="book_close" href="#" onclick="location.href=window.location.pathname;"><img src="/images/close.png" alt="" /></a><span>' . $pageText . '</span></div><div id="book_bg" style="display:block" onclick="location.href=window.location.pathname;"></div>';
...
print '<div id="book_bg" onclick="document.getElementById(\'book_form\').style.display = \'none\';document.getElementById(\'book_bg\').style.display = \'none\';"></div><div id="book_form"><form action="' . $url . '" method="post" onsubmit="return checkForm(this);">' . "\n" .
      ...
      '<p><em> Обращаем Ваше внимание, что приведённая ниже форма — предварительная заявка записи на приём. С Вами свяжется сотрудник службы координации пациентов для подтверждения записи.<br />
...
print '<tr><td style="width: 50%;"><strong>Фамилия:</strong> </td><td style="width: 50%;"><input class="rapid_contact inputbox" type="text" name="rp_last_name" value="'.$CORRECT_LAST_NAME.'" /></td></tr>';

print '<tr><td><strong>Имя:*</strong> </td><td><input class="rapid_contact inputbox" type="text" name="rp_name" value="'.$CORRECT_NAME.'"/></td></tr>';

print '<tr><td><strong>Отчество:</strong> </td><td><input class="rapid_contact inputbox" type="text" name="rp_fathers_name" value="'.$CORRECT_FATHERS_NAME.'" /></td></tr>';
...
print '<tr valign="top"><td><strong>Запись к врачу или на исследование:*</strong> <br /><em>Например: к терапевту; или на УЗИ.</em> </td><td><textarea class="rapid_contact textarea" name="rp_book" rows="4">'.$CORRECT_BOOK.'</textarea></td></tr>';
...
print '<tr><td>Введите число, которое Вы видите на картинке:* </td><td><input class="rapid_contact inputbox ' . $mod_class_suffix . '" type="text" style="width:50px" maxlength="3" name="captcha" value="" />&nbsp;<img src="/libraries/captcha.png" width="120" height="30" border="1" alt="CAPTCHA" style="margin-bottom: 6px" /></td></tr>';
